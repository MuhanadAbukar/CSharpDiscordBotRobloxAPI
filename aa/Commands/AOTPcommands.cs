    ...
    internal class AOTPcommands : BaseCommandModule
    {
        ...
        public async Task real(CommandContext ctx, string placeId, string jobid)
        {
            ...
            foreach (string xd2 in REAL)
            {
                ...
                {
                    Console.WriteLine("Port: \n" + xd2.Replace("\"ServerPort\":", ""));
                    resl += "Port: \n" + xd2.Replace("\"ServerPort\":", "") + "\n";
                }
                else if (xd2.StartsWith("\\\"PlaceId"))
                {
                    var id = xd2.Split(":")[1];
                    Console.WriteLine(id);
                    var z = new HttpClient();
                    var universe2 = await z.GetStringAsync($"https://api.roblox.com/universes/get-universe-containing-place?placeid={id}");
                    System.Threading.Thread.Sleep(1000);
                    universe2 = universe.Replace("\"UniverseId\":", "");
                    var xd = await z.GetStringAsync($"https://develop.roblox.com/v1/universes/{universe2}");
                    System.Threading.Thread.Sleep(1000);
                    var sd2 = xd.Split(',');
                    var blacklist = "";
